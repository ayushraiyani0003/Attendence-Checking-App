async function redisMysqlAttendanceCompare(
    employees,
    redisAttendanceData,
    mysqlAttendanceData,
    groups,
    ...
    try {
        ...
        for (const item of redisAttendanceData) {
            ...
            const formattedDate1 = `${redisDate.getDate()}/${
                redisDate.getMonth() + 1
            }/${redisDate.getFullYear()}`;
            ...
            if (Array.isArray(parsedData)) {
                for (const empData of parsedData) {
                    ...
                    redisDataMap.set(
                        `${formattedDate1}_${group}_${empData.employee_id}`,
                        empData
                    );
                    redisDataMap.set(
                        `${date}_${group}_${empData.employee_id}`,
                        empData
                    );
                }
            } else if (parsedData && parsedData.employee_id) {
                redisDataMap.set(
                    `${formattedDate1}_${group}_${parsedData.employee_id}`,
                    parsedData
                );
                redisDataMap.set(
                    `${date}_${group}_${parsedData.employee_id}`,
                    parsedData
                );
            }
        }
        ...
        for (const employee of mysqlAttendanceData) {
            ...
            const attendanceDate = new Date(attendance_date);
            const formattedDate = `${attendanceDate.getDate()}/${
            ...
            if (!dayOfWeek) {
                dayOfWeek = attendanceDate.toLocaleString("en-us", {
                    weekday: "long",
                });
            ...
            const attendanceRecord = {
                ...
                day: dayOfWeek,
                netHR:
                    network_hours !== undefined && network_hours !== null
                        ? network_hours
                        : 0,
                otHR:
                    overtime_hours !== undefined && overtime_hours !== null
                        ? overtime_hours
                        : 0,
            ...
            redisData =
                redisDataMap.get(`${formattedDate}_${group}_${employee_id}`) ||
            ...
            if (!redisData && groups.length > 1) {
                for (const groupName of groups) {
                    ...
                    redisData =
                        redisDataMap.get(
                            `${formattedDate}_${groupName}_${employee_id}`
                        ) ||
                        redisDataMap.get(
                            `${attendance_date}_${groupName}_${employee_id}`
                        );
            ...
            if (redisData) {
                ...
                if (
                    redisData.network_hours !== undefined &&
                    redisData.network_hours !== null
                ) {
                    attendanceRecord.netHR = parseFloat(
                        redisData.network_hours
                    );
                }

                if (
                    redisData.overtime_hours !== undefined &&
                    redisData.overtime_hours !== null
                ) {
                    attendanceRecord.otHR = parseFloat(
                        redisData.overtime_hours
                    );
                }
        ...
        const resultData = finalAttendanceData.filter(
