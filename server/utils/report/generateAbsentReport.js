...
async function generateAbsentReport(
    finalAttendanceData,
    metricsData,
    month,
    year,
    ...
    if (employeeType === "Faulty Employees") {
        ...
        return {
            success: false,
            message:
                "Faulty Employees report not implemented for absent report",
        };
    }
    ...
    for (const employeeData of employeesData) {
        ...
        if (employeeData && employeeData.dataValues) {
            ...
            employeeName =
                employeeData.dataValues.name || `Employee ${employeeId}`;
        ...
        for (const date of allDates) {
            ...
            const networkHours =
                attendanceByEmployee[employeeId]?.[dateString] || 0;
        ...
        if (absentDays.length > 0) {
            ...
            const standardizedEmployee = {
                dataValues: {
                    ...
                    reporting_group:
                        employeeData.dataValues?.reporting_group || "",
                },
            };
    ...
    const totalAbsences = summaryRow
        .slice(6)
    ...
    worksheet.insertRow(1, [
        `Absent Report (${startDate.format("DD-MM-YYYY")} to ${endDate.format(
            "DD-MM-YYYY"
        )})`,
    ]);
