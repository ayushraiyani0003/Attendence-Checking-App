...
async function generateNightShiftReport(
    finalAttendanceData,
    metricsData,
    numericMonth,
    numericYear,
    ...
    nightShiftEmployees = employeeDetails.filter(
        (emp) =>
    ...
    if (options.includes("hours")) {
        for (let i = 4; i < totalRowsCount; i++) {
            ...
            if (employeeIdx < nightShiftEmployees.length) {
                const employeeId =
                    nightShiftEmployees[employeeIdx].dataValues.employee_id;
                ...
                for (let dateIdx = 0; dateIdx < dateArray.length; dateIdx++) {
                    ...
                    if (attendanceRecord) {
                        // Find metrics data for this employee
                        const employeeMetrics = metricsData.find((metric) => {
                            const empDetail = nightShiftEmployees.find(
                                (emp) =>
