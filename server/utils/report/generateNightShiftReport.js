...
async function generateNightShiftReport(finalAttendanceData, metricsData, numericMonth, numericYear, options, dateRange, employeeType, employeeDetails) {
  ...
  if (employeeType === 'Faulty Employees') {
    ...
    filteredAttendanceData.forEach(record => {
      ...
      const employeeMetrics = metricsData.find(metric => {
        const employeeDetail = employeeDetails.find(emp => emp.dataValues.employee_id === employeeId);
        return employeeDetail && metric.dataValues.punch_code === employeeDetail.dataValues.punch_code;
      });
      
      if (employeeMetrics) {
        const networkHoursArray = JSON.parse(employeeMetrics.dataValues.network_hours);
        const overtimeHoursArray = JSON.parse(employeeMetrics.dataValues.overtime_hours);
        
        // Find the specific date's hours
        const networkHoursObj = networkHoursArray.find(item => item.date === dateStr);
        const overtimeHoursObj = overtimeHoursArray.find(item => item.date === dateStr);
        ...
        if (networkHoursObj && overtimeHoursObj) {
          ...
          if (Math.abs(metricsNetHours - attendanceNetHours) > 0.25 || 
  ...
  const uniqueEmployeeIds = [...new Set(filteredAttendanceData.map(record => record.employee_id))];
  
  // Get employee details for these IDs
  nightShiftEmployees = employeeDetails.filter(emp => 
  ...
  dateArray.forEach(date => {
    ...
    } else if (options === 'remarks' || options.includes('hours') && options.includes('remarks')) {
      headerRow.push(dateStr);
      headerRow.push('');
      headerRow.push('');
      headerRow2.push('Net Hr');
  ...
  // Add employee rows
  nightShiftEmployees.forEach((employee, index) => {
    ...
    const employeeReportingGroup = employee.dataValues.reporting_group || '';
    ...
    dateArray.forEach(date => {
      ...
      const attendanceRecord = filteredAttendanceData.find(record => 
      ...
      const employeeMetrics = metricsData.find(metric => 
        metric.dataValues.punch_code === employee.dataValues.punch_code
      );
      ...
      if (attendanceRecord) {
        ...
        hasNight = attendanceRecord.shift_type === 'N' ? 1 : 0;
        ...
        totalsByDate[formattedDate].count += hasNight;
        ...
        if (employeeMetrics) {
          ...
          if (networkHoursObj && overtimeHoursObj) {
            ...
            if (Math.abs(metricsNetHours - netHr) > 0.25 || 
  ...
  // Calculate grand total of night counts
  ...
  dateArray.forEach(date => {
    ...
    if (options.includes('count') && !options.includes('remarks')) {
      ...
      grandTotalNightCount += totalsByDate[formattedDate].count;
    } else if (options.includes('count') && options.includes('remarks')) {
      totalRow.push(totalsByDate[formattedDate].count);
      grandTotalNightCount += totalsByDate[formattedDate].count;
      totalRow.push('');
  ...
  // Add grand totals
  if (options.includes('count') && !options.includes('hours')) {
    totalRow.push(grandTotalNightCount);
  } else {
  ...
  for (let i = 1; i <= colCount; i++) {
    ...
    if (i <= 6) {
      switch (i) {
        case 1: col.width = 6; break;  // Sr. No.
        case 2: col.width = 10; break; // Punch Code
        case 3: col.width = 30; break; // Name
        case 4: col.width = 18; break; // Designation
        case 5: col.width = 18; break; // Department
        case 6: col.width = 18; break; // Reporting Group
      }
    } 
    // Hours and remarks columns
      ...
      if (dateIndex < dateArray.length) {
        // For date columns
        if (options.includes('hours')) {
          if (columnTypeIndex === 0) { // Net Hr
            col.width = 8;
          } else if (columnTypeIndex === 1) { // OT Hr
            col.width = 8;
          } else if (columnTypeIndex === 2) { // Comment
            col.width = 20;
          }
        } else if (options.includes('count')) {
          if (columnTypeIndex === 0) { // Count
            col.width = 10;
          } else if (columnTypeIndex === 1) { // Comment
            col.width = 20;
          }
        }
      } else {
  ...
  // First apply borders to all cells in the data area
  for (let i = 2; i <= totalRowsCount; i++) {
    for (let j = 1; j <= totalColumnsCount; j++) {
      ...
      if (i > 3 && ((options.includes('hours') && (j > 6 && j <= totalColumnsCount)))) {
        const colOffset = (j - 7) % (options.includes('remarks') ? 3 : 2);
        if (colOffset === 0 || colOffset === 1) { // Net Hr or OT Hr columns
  ...
  for (let i = 2; i <= totalRowsCount; i++) {
    ...
    // Right edge
    worksheet.getCell(i, totalColumnsCount).border.right = { style: 'medium' };
  }
  ...
  for (let j = 1; j <= totalColumnsCount; j++) {
    ...
    worksheet.getCell(totalRowsCount, j).border.bottom = { style: 'medium' };
  }
  ...
  if (options.includes('hours')) {
    for (let i = 4; i < totalRowsCount; i++) {
      ...
      if (employeeIdx < nightShiftEmployees.length) {
        const employeeId = nightShiftEmployees[employeeIdx].dataValues.employee_id;
        ...
        for (let dateIdx = 0; dateIdx < dateArray.length; dateIdx++) {
          ...
          if (attendanceRecord) {
            // Find metrics data for this employee
            const employeeMetrics = metricsData.find(metric => {
              const empDetail = nightShiftEmployees.find(emp => emp.dataValues.employee_id === employeeId);
              return empDetail && metric.dataValues.punch_code === empDetail.dataValues.punch_code;
            });
            ...
            if (employeeMetrics) {
              ...
              if (networkHoursObj && overtimeHoursObj) {
                const metricsNetHours = parseFloat(networkHoursObj.hours);
                const metricsOtHours = parseFloat(overtimeHoursObj.hours);
                const attendanceNetHours = attendanceRecord.network_hours || 0;
                const attendanceOtHours = attendanceRecord.overtime_hours || 0;
                
                // Calculate column positions for Net Hr and OT Hr
                let startCol = 7;
                for (let d = 0; d < dateIdx; d++) {
                  startCol += options.includes('remarks') ? 3 : 2;
                }
                
                // Highlight Net Hr cell if faulty
                if (Math.abs(metricsNetHours - attendanceNetHours) > 0.25) {
                  const netHrCell = worksheet.getCell(i, startCol);
                  netHrCell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                ...
                if (Math.abs(metricsOtHours - attendanceOtHours) > 0.25) {
                  const otHrCell = worksheet.getCell(i, startCol + 1);
                  otHrCell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'FFFFCCCC' } // Light red
  ...
  const fileName = `${reportType}_${employeeType.replace(/\s+/g, '_')}_${fileStartDate}_to_${fileEndDate}_${optionsStr}.xlsx`;
...
module.exports = generateNightShiftReport;