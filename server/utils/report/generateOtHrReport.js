...
async function generateOtHrReport(finalAttendanceData, metricsData, month, year, options, dateRange, employeeType, employeeDetails) {
  ...
  if (employeeType === "New Employees") {
    ...
    filteredEmployees = employeesData.filter(emp => {
      ...
      for (const date of allDates) {
        ...
        const actualHours = attendanceByEmployee[employeeId]?.[dateStr]?.overtimeHours || 0;
        const expectedHours = metricsByDate[dateStr]?.overtimeHours || 0;
  ...
  worksheet.insertRow(1, [`OT Hours Report (${startDate.format('DD-MM-YYYY')} to ${endDate.format('DD-MM-YYYY')})`]);
  ...
  if (includeRemarks) {
    ...
    filteredEmployees.forEach(employee => {
      ...
      allDates.forEach(date => {
        ...
        const hours = attendanceByEmployee[employeeId]?.[dateStr]?.overtimeHours || 0;
