...
async function generateNetHrReport(finalAttendanceData, metricsData, month, year, options, dateRange, employeeType, employeeDetails) {
  ...
  finalAttendanceData.forEach(attendance => {
    ...
    attendanceByEmployee[employeeId][attendanceDate] = {
      networkHours: attendance.network_hours,
  ...
  if (metricsData && metricsData.dataValues) {
    try {
      // Parse the network_hours from JSON string
      const networkHoursJson = JSON.parse(metricsData.dataValues.network_hours);
      
      networkHoursJson.forEach(item => {
        ...
        metricsByDate[dateStr].networkHours = parseFloat(item.hours) || 0;
      });
      
      // Parse overtime_hours if available
      if (metricsData.dataValues.overtime_hours) {
        const overtimeHoursJson = JSON.parse(metricsData.dataValues.overtime_hours);
        ...
        overtimeHoursJson.forEach(item => {
          ...
          metricsByDate[dateStr].overtimeHours = parseFloat(item.hours) || 0;
        });
      }
    } catch (error) {
  ...
  if (employeeType === "New Employees") {
    ...
    // Filter employees with discrepancies between actual and expected network hours
    filteredEmployees = employeesData.filter(emp => {
      if (!emp || !emp.dataValues) return false;
      ...
      for (const date of allDates) {
        ...
        const actualHours = attendanceByEmployee[employeeId]?.[dateStr]?.networkHours || 0;
        const expectedHours = metricsByDate[dateStr]?.networkHours || 0;
  ...
  const worksheet = workbook.addWorksheet('Network Hours Report');
  ...
  // Add date columns
  if (includeRemarks) {
    // For each date, add two columns: Net Hr and Comment
    allDates.forEach(date => {
      columns.push({ header: date.format('DD-MM'), width: 10 }); // Net Hr column
      columns.push({ header: '', width: 20 }); // Comment column - header will be set later
    });
    // Add total column
    columns.push({ header: 'Total', width: 15 });
  } else {
    // Just add one column per date for Net Hr
    allDates.forEach(date => {
  ...
  worksheet.insertRow(1, [`Network Hours Report (${startDate.format('DD-MM-YYYY')} to ${endDate.format('DD-MM-YYYY')})`]);
  ...
  // If remarks are included, add a second header row for Net Hr / Comment labels
  if (includeRemarks) {
    ...
    // Set Net Hr and Comment labels
    let columnIndex = 7; // Start after employee info columns
    allDates.forEach(() => {
      subheaderRow.getCell(columnIndex).value = 'Net Hr';
    ...
    // Merge date headers in row 2 to span Net Hr and Comment columns
  ...
  filteredEmployees.forEach((employee, index) => {
    ...
    if (includeRemarks) {
      ...
      allDates.forEach(date => {
        const dateStr = date.format('YYYY-MM-DD');
        const actualHours = attendanceByEmployee[employeeId]?.[dateStr]?.networkHours || 0;
        const comment = attendanceByEmployee[employeeId]?.[dateStr]?.comment || '';
        
        totalHours += parseFloat(actualHours) || 0;
        
        rowData.push(actualHours.toFixed(2)); // Net hours
    ...
    row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
      ...
      if (!includeRemarks && colNumber >= 7) {
        ...
        // No background color for hours-only report as requested
      } 
      // With remarks, handle hours and comments separately
        ...
        if (isHoursColumn || isTotalColumn) {
          ...
          if (cell.value && cell.value.toString().toLowerCase().startsWith('site')) {
            cell.fill = {
              type: 'pattern',
              pattern: 'solid',
  ...
  const summaryRowData = ['Total Network Hours', '', '', '', '', ''];
  const totalsByDate = {};
  ...
  if (includeRemarks) {
    ...
    filteredEmployees.forEach(employee => {
      ...
      allDates.forEach(date => {
        ...
        const hours = attendanceByEmployee[employeeId]?.[dateStr]?.networkHours || 0;
  ...
  const grandTotal = Object.values(totalsByDate).reduce((sum, val) => sum + val, 0);
  ...
  summaryRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
    ...
    if ((includeRemarks && (colNumber - 7) % 2 === 0 && colNumber >= 7) || 
  ...
  const employeeTypeStr = typeof employeeType === 'string' ? employeeType.replace(/\s+/g, '_').toLowerCase() : 'all';
  
  const filename = `net_hr_report_${startDateStr}_to_${endDateStr}_${employeeTypeStr}_${optionsStr}.xlsx`;
...
module.exports = generateNetHrReport;