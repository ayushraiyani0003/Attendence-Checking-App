...
async function generateNetHrReport(
    finalAttendanceData,
    metricsData,
    month,
    year,
    ...
    if (metricsData && metricsData.dataValues) {
        try {
            ...
            const networkHoursJson = JSON.parse(
                metricsData.dataValues.network_hours
            );
            ...
            networkHoursJson.forEach((item) => {
                ...
                metricsByDate[dateStr].networkHours =
    ...
    if (Array.isArray(employeeDetails) && employeeDetails.length > 0) {
        // Filter employees to include only those with active status
    ...
    if (employeeType === "New Employees") {
        ...
        filteredEmployees = employeesData.filter((emp) => {
            ...
            for (const date of allDates) {
                ...
                const actualHours =
                    attendanceByEmployee[employeeId]?.[dateStr]?.networkHours ||
                    0;
    ...
    worksheet.insertRow(1, [
        `Network Hours Report (${startDate.format(
            "DD-MM-YYYY"
