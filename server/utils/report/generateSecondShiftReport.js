...
async function generateSecondShiftReport(
    finalAttendanceData,
    metricsData,
    numericMonth,
    numericYear,
    ...
    secondShiftEmployees = employeeDetails.filter(
        (emp) =>
    ...
    // Add employee rows
    secondShiftEmployees.forEach((employee, index) => {
        ...
        dateArray.forEach((date) => {
            ...
            if (attendanceRecord) {
                ...
                hasDay = attendanceRecord.shift_type === "2S" ? 1 : 0;
    ...
    if (options.includes("hours")) {
        for (let i = 4; i < totalRowsCount; i++) {
            ...
            if (employeeIdx < secondShiftEmployees.length) {
                const employeeId =
                    secondShiftEmployees[employeeIdx].dataValues.employee_id;
                ...
                for (let dateIdx = 0; dateIdx < dateArray.length; dateIdx++) {
                    ...
                    if (attendanceRecord) {
                        // Find metrics data for this employee
                        const employeeMetrics = metricsData.find((metric) => {
                            const empDetail = secondShiftEmployees.find(
                                (emp) =>
    ...
    // Generate filename in the format similar to "second_report_All Employees_20250401_to_20250407.xlsx"
    const reportType = "second_report";
...
module.exports = generateSecondShiftReport;
