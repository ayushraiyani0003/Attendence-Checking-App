...
async function generateEveningShiftReport(finalAttendanceData, metricsData, numericMonth, numericYear, options, dateRange, employeeType, employeeDetails) {
  ...
  eveningShiftEmployees = employeeDetails.filter(emp => 
  ...
  eveningShiftEmployees.forEach((employee, index) => {
    ...
    dateArray.forEach(date => {
      ...
      if (attendanceRecord) {
        ...
        hasEvening = attendanceRecord.shift_type === 'E' ? 1 : 0;
  ...
  // Calculate grand total of evening counts
  ...
  if (options.includes('hours')) {
    for (let i = 4; i < totalRowsCount; i++) {
      ...
      if (employeeIdx < eveningShiftEmployees.length) {
        const employeeId = eveningShiftEmployees[employeeIdx].dataValues.employee_id;
        ...
        for (let dateIdx = 0; dateIdx < dateArray.length; dateIdx++) {
          ...
          if (attendanceRecord) {
            // Find metrics data for this employee
            const employeeMetrics = metricsData.find(metric => {
              const empDetail = eveningShiftEmployees.find(emp => emp.dataValues.employee_id === employeeId);
  ...
  const reportType = 'evening_report';
