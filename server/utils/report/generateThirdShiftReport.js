...
async function generateThirdShiftReport(
    finalAttendanceData,
    metricsData,
    numericMonth,
    numericYear,
    ...
    thirdShiftEmployees = employeeDetails.filter(
        (emp) =>
    ...
    // Add employee rows
    thirdShiftEmployees.forEach((employee, index) => {
        ...
        dateArray.forEach((date) => {
            ...
            if (attendanceRecord) {
                ...
                hasDay = attendanceRecord.shift_type === "3S" ? 1 : 0;
    ...
    if (options.includes("hours")) {
        for (let i = 4; i < totalRowsCount; i++) {
            ...
            if (employeeIdx < thirdShiftEmployees.length) {
                const employeeId =
                    thirdShiftEmployees[employeeIdx].dataValues.employee_id;
                ...
                for (let dateIdx = 0; dateIdx < dateArray.length; dateIdx++) {
                    ...
                    if (attendanceRecord) {
                        // Find metrics data for this employee
                        const employeeMetrics = metricsData.find((metric) => {
                            const empDetail = thirdShiftEmployees.find(
                                (emp) =>
    ...
    // Generate filename in the format similar to "third_report_All Employees_20250401_to_20250407.xlsx"
    const reportType = "third_report";
...
module.exports = generateThirdShiftReport;
