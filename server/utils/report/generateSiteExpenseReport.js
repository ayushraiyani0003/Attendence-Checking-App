 ...
 * @param {string} options - Report optionss: "count", "hours", "remarks", or combinations
 ...
 */
async function generateSiteExpenseReport(
    finalAttendanceData,
    metricsData,
    month,
    year,
    ...
    // console.log(finalAttendanceData);
    ...
    let filteredEmployees = filterEmployeesByType(
        employeeDetails,
        employeeType,
        finalAttendanceData,
        metricsData
    ...
    filteredEmployees = filterEmployeesWithSiteVisits(
        filteredEmployees,
        finalAttendanceData
    );
    ...
    const { dateHeaders, dateToWeekdayMap } = generateDateHeaders(
        startDate,
        endDate
    );

    // Continue with report generation...
    setupWorksheetHeaders(worksheet, dateHeaders, options);

    // Aggregate data - now passing dateToWeekdayMap
    const { totals, siteCountTotal, netHrCountTotal, otHrCountTotal } =
        populateDataRows(
            worksheet,
            filteredEmployees,
            attendanceMap,
    ...
    // Apply styles
    styleWorksheet(worksheet, options, dateHeaders.length);
    ...
    const sanitizedEmployeeType =
        typeof employeeType === "string"
            ? employeeType.replace(/\s+/g, "")
            : "AllEmployees";
    const sanitizedOption =
        typeof options === "string" ? options.replace(/\s+/g, "") : "default";
...
function highlightWednesdayHeaders(
    worksheet,
    dateHeaders,
    dateToWeekdayMap,
    options
) {
    const headerRow = options === "count" ? 1 : 2;
    let colIndex = 5; // Start after employee details columns

    for (const dateHeader of dateHeaders) {
        if (dateToWeekdayMap[dateHeader] === 3) {
            // 3 is Wednesday
            if (options === "count") {
...
function filterEmployeesByType(
    ...
    if (!employeeType || employeeType === "All Employees") {
        ...
        if (!metricsData || metricsData.length === 0) {
            console.warn(
                "No metrics data available for faulty employees analysis"
            );
            return []; // Return empty array instead of filtering
        }
        return employees.filter((employee) => {
            ...
            // In filterEmployeesByType() function
            ...
            if (!employeeMetrics) {
                return false; // This eliminates ALL employees when metrics is empty
            }
            ...
            for (const attendance of employeeAttendance) {
                ...
                const metricsDataValues =
                    employeeMetrics.dataValues || employeeMetrics;
                const networkHoursFromMetrics = getHoursFromMetricsJson(
                    metricsDataValues.network_hours,
                    attendanceDate
                );
                const otHoursFromMetrics = getHoursFromMetricsJson(
                    metricsDataValues.overtime_hours,
                    attendanceDate
                );

                const networkHoursDiff = Math.abs(
                    attendance.network_hours - networkHoursFromMetrics
                );
                const otHoursDiff = Math.abs(
                    attendance.overtime_hours - otHoursFromMetrics
                );
...
function filterEmployeesWithSiteVisits(employees, attendanceData) {
    ...
    return employees.filter((employee) => {
        ...
        const employeeId = employee.dataValues
            ? employee.dataValues.employee_id
 ...
 * @param {string} options - Report options
...
function populateDataRows(
    ...
    // Determine starting row based on options (accounting for headers)
    const startRow = options === "count" ? 1 : 2;
    ...
    employees.forEach((employee, index) => {
        ...
        for (const dateHeader of dateHeaders) {
            ...
            const attendance = attendanceMap[employeeId]
                ? attendanceMap[employeeId][dateHeader]
                : null;
            const hasSiteVisit =
                attendance &&
                isSiteComment(
                    attendance.comment,
                    attendance.network_hours,
                    attendance.overtime_hours
                );

            // Initialize date totals if not exists
            if (!dateTotals.netHours[dateHeader])
                dateTotals.netHours[dateHeader] = 0;
            if (!dateTotals.otHours[dateHeader])
                dateTotals.otHours[dateHeader] = 0;
            if (!dateTotals.siteCount[dateHeader])
                dateTotals.siteCount[dateHeader] = 0;
            if (!dateTotals.netCount[dateHeader])
                dateTotals.netCount[dateHeader] = 0;
            if (!dateTotals.otCount[dateHeader])
                dateTotals.otCount[dateHeader] = 0;

            // Only get hours and show remarks when there's a site comment
            const netHours =
                hasSiteVisit && attendance ? attendance.network_hours || 0 : 0;
            const otHours =
                hasSiteVisit && attendance ? attendance.overtime_hours || 0 : 0;
            const remark =
                hasSiteVisit && attendance && attendance.comment
                    ? attendance.comment
                    : "";
            ...
            // Handle different report optionss
            if (options === "count") {
        ...
        if (
            (options === "hours" ||
                options === "remarks" ||
                options === "hours and remarks") &&
            employeeMetrics &&
            metricsData.length > 0 // Add this check
        ) {
...
function applyConditionalFormatting(
    ...
    for (const dateHeader of dateHeaders) {
        ...
        if (attendance && hasSiteVisit) {
            const expectedNetHours = getHoursFromMetricsJson(
                employeeMetrics.network_hours,
                day
            );
            const expectedOTHours = getHoursFromMetricsJson(
                employeeMetrics.overtime_hours,
                day
            );
        ...
        // If no site visit, still apply Wednesday highlighting
            ...
            } else if (options === "remarks" || options === "") {
                worksheet.getCell(rowIndex, colIndex).fill = {
                    type: "pattern",
                    pattern: "solid",
                    fgColor: { argb: "FFFFFFCF" }, // Light red/pink
        ...
        // Advance column index based on options
        if (options === "hours") {
 ...
 * @param {number} dateCount - Number of dates
 */
function styleWorksheet(worksheet, options, dateCount) {
    // Set header style
    ...
    // Set date column widths based on options
    let colIndex = 5;
