    ...
    try {
        ...
        for (const date of dates) {
            ...
            const attendanceData = {
                ...
                shift_type: "GS", // Default shift type
                network_hours: 0,
                overtime_hours: 0,
                comment: "",
            };
            ...
            const redisAvailability = await checkDataAvailableInRedis(
                date,
                reportingGroup
            );
...
function compareAttendanceData(metricsData, finalAttendanceData) {
    ...
    const matchingEmployees = finalAttendanceData.attendance.filter(
        (emp) => emp.punchCode === metricsPunchCode
    );
...
function compareAttendanceDataForTotal(metricsDataArray, finalAttendanceData) {
    ...
    metricsDataArray.forEach((metricsData) => {
        ...
        matchingEmployees.forEach((employee) => {
            ...
            // Calculate differences
            const netHRDiff = (
                totalAttendanceNetHR - totalMetricsNetHR
            ).toFixed(2);
            const otHRDiff = (totalAttendanceOtHR - totalMetricsOtHR).toFixed(
                2
            );
