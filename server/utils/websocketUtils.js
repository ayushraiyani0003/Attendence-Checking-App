...
const {
    ...
    processAllMetricsData,
} = require("./quickFunction");
const {
...
const {
    redisMysqlAttendanceCompare,
} = require("./redisMysqlAttendenceCompare");
const {
    getLockStatusDataForMonthAndGroup,
    setStatusFromDateGroup,
} = require("../services/groupAttendenceLockServices");
                ...
                try {
                    ...
                    // Special case for lock status updates - ensure all users in target groups receive this
                        ...
                        // For users, check if they're in one of the target groups
                            ...
                            const clientGroups = Array.isArray(
                                targetClientInfo.userGroup
                            )
                                ? targetClientInfo.userGroup
                                : [targetClientInfo.userGroup];
                    ...
                    // Normal broadcasts
                        ...
                        // For users or admins in restricted broadcasts
                            ...
                            // For general broadcasts
                                ...
                                const broadcastGroups = Array.isArray(
        ...
        ws.on("message", async (message) => {
            try {
                ...
                if (data.user) {
                    ...
                    if (data.user.userReportingGroup)
                    ...
                    if (data.month) {
                        const { year, month } = convertMonthToYearMonthFormat(
                            data.month
                        );
                ...
                switch (data.action) {
                    ...
                    case "setUserInfo":
                        ...
                        if (data.month) {
                            const { year, month } =
                                convertMonthToYearMonthFormat(data.month);
                            clientInfo.selectedMonth = month;
                            clientInfo.selectedYear = year;
                        }
                    ...
                    default:
                        console.warn(
                            `Unhandled action from client ${id}:`,
                            data.action
                        );
                        ws.send(
                            JSON.stringify({
                ...
                console.error(
                    `Error processing message from client ${id}:`,
                    error
                );
                ws.send(
                    JSON.stringify({
...
async function handleAttendanceDataRetrieval(ws, data) {
    try {
        ...
        const mysqlAttendanceData = await getEmployeesAttendanceByMonthAndGroup(
            group,
            year,
            month
        );
        ...
        const redisAttendanceData = await getRedisAttendanceData(
            year,
            month,
            group
        );
        ...
        const lockStatusData = await getLockStatusDataForMonthAndGroup(
            group,
            month,
            year
        );
        ...
        const metricsAttendanceData = await fetchMetricsForMonthYear(
            month,
            year
        );
        ...
        const metricsAttendanceDifference = await processAllMetricsData(
            metricsAttendanceData,
            finalAttendanceData
        );
...
async function handleAttendanceUpdate(ws, data, broadcastToClients) {
    try {
        ...
        if (
            ...
            !userReportingGroups.includes(data.reportGroup)
        ) {
        ...
        broadcastToClients(
            ...
            ws,
            isAdminUpdate ? null : [data.reportGroup]
        );
...
async function saveDataRedisToMysql(ws, data, broadcastToClients) {
    try {
        ...
        if (!redisAttendanceData) {
            ...
            throw new Error(
                "No attendance data found in Redis for the specified month/year and groups"
            );
        }
        ...
        if (Array.isArray(redisAttendanceData)) {
            ...
            redisAttendanceData.attendance &&
            Array.isArray(redisAttendanceData.attendance)
        ) {
            ...
            if (
                !redisAttendanceData.date &&
                redisAttendanceData.key &&
                typeof redisAttendanceData.key === "string"
            ) {
                const dateMatch =
                    redisAttendanceData.key.match(/(\d{4}-\d{2}-\d{2})$/);
                if (dateMatch && dateMatch[1]) {
                    date = dateMatch[1];
                }
            }
        ...
        if (formattedRedisData.length === 0) {
            console.warn(
                "No valid attendance records found after processing Redis data"
            );
            throw new Error(
                "No valid attendance records found after processing Redis data"
            );
        ...
        const updateResult = await updateEmployeesDetailsFromRedis(
            formattedRedisData,
            data.user
        );
        ...
        const successfulUpdates = statusResults.filter(
        ...
        if (failedUpdates.length > 0) {
            ...
            failedUpdates.forEach((failure) => {
                console.warn(
                    `Failed date: ${failure.date}, Error: ${failure.error}`
                );
            });
        }
...
async function lockUnlockStatusToggle(ws, data, broadcastToClients) {
    try {
        ...
        if (data.user.role !== "admin") {
            console.error(
                "Unauthorized attempt to change lock status by:",
                data.user.role
            );
 ...
 */
async function processUnlockRequest(
    ws,
    formattedDateString,
    groupList,
    user,
    ...
    const availableStatus = await checkDataAvailableInRedis(
        formattedDateString,
        groupList
    );
    ...
    if (
        ...
        mysqlAttendanceData.records.length === 0
    ) {
        ws.send(
            JSON.stringify({
                type: "lockUnlockStatus",
    ...
    const result = await setStatusFromDateGroup(
        groupList,
        formattedDateString,
        "unlocked",
        user
    ...
    broadcastToClients(
        ...
        false,
        false
    ); // Set restrictedBroadcast to false to ensure users get the update
 ...
 * @param {string} monthYear - Month/Year string for filtering
 */
async function processLockRequest(
    ws,
    date,
    formattedDateString,
    groupList,
    ...
    const deleteRedisKey = await deleteRedisGroupKeysForSelectedDate(
        date,
        groupList
    );
