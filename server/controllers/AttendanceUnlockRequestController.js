...
} = require("../utils/getRedisAttendenceData");
const {
...
class AttendanceUnlockRequestController {
        ...
        try {
            ...
            if (!requestedById || !requestedBy || !startDate) {
                return res.status(400).json({
                    success: false,
                    message:
                        "Required fields missing: requestedById, requestedBy, and date information are mandatory",
                });
            }
        ...
        try {
            ...
            if (!requestId || !statusBy || !status || !user) {
                return res.status(400).json({
                    success: false,
                    message:
                        "Required fields missing: requestId, statusBy, and status are mandatory",
                });
            }
            ...
            if (status === "approved" && user) {
                ...
                const requestDetails = await this.requestService.getRequestById(
                    requestId
                );
                ...
                const requestedUser = allUsers.find(
                    (u) => u.id === requestedById
                );
                ...
                if (dateRange && dateRange.startDate && dateRange.endDate) {
                    ...
                    for (
                        ...
                        datesToProcess.push(
                            this.formatDateToYYYYMMDD(new Date(d))
                        );
                    }
                }
                ...
                try {
                    for (const dateToProcess of datesToProcess) {
                        ...
                        const unavailableGroups = groupList.filter(
                            (groupName) =>
                                !availableStatus.groups[groupName]?.available
                        );

                        // Fetch attendance data for unavailable groups
                        const mysqlAttendanceData =
                            await getAttendanceSelectedGroupDateMysql(
                                unavailableGroups,
                                dateToProcess
                            );
                        ...
                        const redisKeys =
                            storeAttendanceInRedis(mysqlAttendanceData);
    ...
    async getFilteredRequests(req, res) {
        try {
            ...
            if (requestedById)
                filters.requestedById = parseInt(requestedById, 10);

            // Call service to get filtered requests
            const requests = await this.requestService.getFilteredRequests(
                filters
            );
