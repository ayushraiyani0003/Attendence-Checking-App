...
exports.getSettings = [authenticateJWT, isAdmin, async (req, res) => {
  ...
  try {
    ...
    return res.json({ departments, designations, reportingGroups });
  } catch (err) {
    console.error('Error fetching settings:', err);
    return res.status(500).json({ error: "Error fetching settings. Please try again later." });
  }
}];

// Replace all Departments and Designations
exports.updateSettings = [authenticateJWT, isAdmin, async (req, res) => {
  const transaction = await db.sequelize.transaction();  // Start a transaction

  try {
    const { departments, designations, reportingGroups } = req.body;
    ...
    if (departments && Array.isArray(departments) && departments.length > 0) {
      ...
      if (validDepartments.length > 0) {
        ...
        await db.Department.bulkCreate(validDepartments, { transaction });
      }
    }
    ...
    if (designations && Array.isArray(designations) && designations.length > 0) {
      ...
      const validDesignations = designations
        ...
        .map(desig => ({ designation_name: desig.designation_name }));  // Only keep the designation_name field

      if (validDesignations.length > 0) {
        // Add the new designations in bulk within the transaction
        await db.Designation.bulkCreate(validDesignations, { transaction });
      }
    }
    ...
    if( reportingGroups && Array.isArray(reportingGroups) && reportingGroups.length > 0) {
      ...
      if (validReportingGroups.length > 0) {
        ...
        await db.ReportingGroup.bulkCreate(validReportingGroups, { transaction });
      }
    }
    ...
    return res.status(500).json({ error: "Error updating settings. Please try again later." });
  }
}];
...
exports.deleteDepartment = [authenticateJWT, isAdmin, async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Error deleting department. Please try again later." });
  }
}];

// Delete Designation
exports.deleteDesignation = [authenticateJWT, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const deletedCount = await db.Designation.destroy({ where: { id } });
    ...
    return res.status(500).json({ error: "Error deleting designation. Please try again later." });
  }
}];

// delete reporting group
exports.deleteReportingGroup = [authenticateJWT, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;  // Get the id from the URL params
    const deletedCount = await ReportingGroup.destroy({ where: { id } });

    if (deletedCount === 0) {
      return res.status(404).json({ message: "Reporting Group not found" });
    }

    return res.status(200).json({ message: "Reporting Group deleted successfully" });
  } catch (err) {
    console.error('Error deleting reporting group:', err);
    return res.status(500).json({ error: "Error deleting reporting group. Please try again later." });
  }
}];
