...
export const getDashboardGraphs = async (month, year) => {
    try {
        ...
        const response = await axios.get(`${API_URL}/dashboard/graphs`, {
            params,
        });
    ...
    try {
        ...
        if (isExcelFile || isZipFile) {
            ...
            if (contentDisposition) {
                ...
                const cleanReportType = reportType
                    .toLowerCase()
                    .replace(/\s+/g, "_");
        ...
        if (error.response) {
            ...
            if (error.response.data instanceof Blob) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    reader.onerror = () =>
                        reject(new Error("Error reading error response"));
                ...
                throw new Error(
                    error.response.data.message ||
                        "Error generating report asdfghj"
                );
            }
        }
        throw error;
