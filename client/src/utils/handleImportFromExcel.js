                ...
                try {
                        ...
                        try {
                            ...
                            if (row["Resign Date"]) {
                                ...
                                // If it's a date object from Excel
                                    ...
                                    employeeData.resign_date = dayjs(
                                        row["Resign Date"]
                                    ).format("YYYY-MM-DD");
                                }
                            } else {
                            ...
                            if (
                                !employeeData.name ||
                                !employeeData.punch_code ||
                                !employeeData.department ||
                                !employeeData.designation ||
                                !employeeData.reporting_group
                            ) {
                                console.warn(
                                    "Missing required fields for row:",
                                    row
                                );
                                errorCount++;
                                continue;
                            ...
                            const existingEmployee = employees.find(
                                (emp) =>
                                    emp.employee_id === employeeData.employeeId
                            );
                    ...
                    if (addedCount > 0 || updatedCount > 0) {
                        notification.success({
                            ...
                            description: `Successfully imported ${addedCount} new employees and updated ${updatedCount} existing employees. ${
                                errorCount > 0
                                    ? `Failed to process ${errorCount} entries.`
                                    : ""
                            }`,
                        ...
                        notification.error({
                            ...
                            description:
                                "No employees were imported or updated. Please check your Excel file format.",
                            placement: "bottomRight",
                        });
                    }
                } catch (error) {
