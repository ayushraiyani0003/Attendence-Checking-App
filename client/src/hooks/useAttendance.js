...
export const useAttendance = (user, monthYear, ws, send, selectedGroup) => {
            ...
            switch (data.action || data.type) {
                case "attendanceData":
                    ...
                    if (
                        ...
                        Array.isArray(sortedAttendanceData)
                    ) {
                        sortedAttendanceData = sortedAttendanceData.map(
                            (employee) => {
                                if (
                                    employee.attendance &&
                                    ...
                                    const sortedAttendance = [
                                        ...
                                        const dateA = convertDateForSorting(
                                            a.date
                                        );
                                        const dateB = convertDateForSorting(
                                            b.date
                                        );
                        ...
                        sortedAttendanceData =
                            sortEmployeesByCustomOrder(sortedAttendanceData);
                    }
                    ...
                    if (
                        data.metricsAttendanceDifference &&
                        Array.isArray(data.metricsAttendanceDifference)
                    ) {
                        let totalMistakes = 0;
                    ...
                    const hasUnlockedDate = data.lockStatus?.some(
                ...
                case "attendanceUpdated":
                    ...
                    setAttendanceData((prevData) => {
                        const updatedData = prevData.map((row) => {
                            if (row.id === data.updateDetails.employeeId) {
                                ...
                                const updatedAttendance = row.attendance.map(
                                    (att) =>
                                        att.date === data.updateDetails.editDate
                                            ? {
                                                  ...att,
                                                  [data.updateDetails.field]:
                                                      data.updateDetails
                                                          .newValue,
                                              }
                                            : att
                    ...
                    if (
                        !data.updateDetails.updatedBy ||
                        data.updateDetails.updatedBy !== user.name
                    ) {
                        toast.info(
                            `${data.updateDetails.field} updated by ${
                                data.updateDetails.updatedBy || "another user"
                            }`
                        );
                    }
                    break;
                ...
                case "attendanceUpdateResult":
                    ...
                    if (data.success) {
                        ...
                        if (
                            ...
                            data.details
                                .toLowerCase()
                                .includes("no existing data")
                        ) {
                            toast.error(
                                "This attendance record is locked. Please request an admin to unlock it before making changes."
                            );
                        } else {
                            toast.error(
                                `Update failed: ${
                ...
                case "lockStatusUpdate":
                    ...
                    setAttendanceData((prevData) => {
                        ...
                        const targetGroups = Array.isArray(data.groups)
                            ? data.groups
                            : [data.groups];
                        let updateCount = 0;
                        ...
                        for (let i = 0; i < updatedData.length; i++) {
                            ...
                            if (
                                employee.reporting_group &&
                                targetGroups.includes(employee.reporting_group)
                            ) {
                    ...
                    setLockStatusData((prevLockStatus) => {
                        ...
                        const dateIndex = prevLockStatus.findIndex(
                            (item) =>
                                item.date === updateDate ||
                                item.dateFormattedString === updateDate
                        );
                    ...
                    toast.info(
                        `Records ${data.status} by ${
                            data.changedBy || "an administrator"
                        }`
                    );
                    break;
                ...
                case "lockUnlockStatusChanged":
                    ...
                    setAttendanceData((prevData) => {
                        const updatedData = prevData.map((row) => {
                            ...
                            if (
                                data.groups &&
                                data.groups.includes(row.reporting_group)
                            ) {
    ...
    const getFilteredData = useCallback(() => {
        ...
        let filteredData = attendanceData.map((item) => {
            ...
            if (itemCopy.attendance && Array.isArray(itemCopy.attendance)) {
                ...
                const startDate = startParsed
                    ? startParsed.format("YYYY-MM-DD")
                    : null;
                const endDate = endParsed
                    ? endParsed.format("YYYY-MM-DD")
                    : null;
        ...
        if (view !== "all") {
            if (view === "diff") {
                ...
                const punchCodesWithDiff = MetrixDiffData.filter(
                    (item) =>
                ...
                filteredData = filteredData.filter(
                    (item) => item.punchCode?.toLowerCase() === "new"
                );
            } else if (view === "night") {
                filteredData = filteredData.filter((item) => {
                    // Check if the employee has an attendance array
    ...
    const handleCellDataUpdate = (rowIndex, date, field, value) => {
        ...
        const columnIndex = updatedEmployee.attendance.findIndex(
            (att) => att.date === date
        );
        ...
        if (String(originalValue).trim() !== String(value).trim()) {
            // the vause is in the string thats why problem acured.
            ...
            toast.info(`Updating ${field} for ${updatedEmployee.name}...`, {
                autoClose: 2000,
            });
        ...
        if (user.role === "admin") {
            ...
            if (firstConfirm) {
                const secondConfirm = window.confirm(
                    "Please confirm once more that you want to save these changes to the database."
                );
