...
const useEmployeeOrder = (userReportingGroup) => {
    ...
    const filterActiveEmployees = useCallback((employeeList) => {
        return employeeList.filter(
            (employee) =>
                employee.status === "active" ||
                employee.status === "Active" ||
                !employee.status // Handle cases where status might be undefined/null
        );
    }, []);
    ...
    }, [employees, applySavedOrder, filterActiveEmployees]);
        ...
        if (isFiltering && searchText.trim() !== "") {
            const filtered = orderedEmployees.filter(
                (emp) =>
                    ...
                    (emp.punch_code?.toLowerCase() || "").includes(
                        searchText.toLowerCase()
                    ) ||
                    (emp.department?.toLowerCase() || "").includes(
                        searchText.toLowerCase()
                    ) ||
                    (emp.designation?.toLowerCase() || "").includes(
                        searchText.toLowerCase()
                    ) ||
                    (emp.reporting_group?.toLowerCase() || "").includes(
                        searchText.toLowerCase()
                    )
            );
    ...
    return {
        ...
        // Added for debugging active employee filtering
        totalEmployeeCount: employees.length,
        activeEmployeeCount: orderedEmployees.length,
    };
};
